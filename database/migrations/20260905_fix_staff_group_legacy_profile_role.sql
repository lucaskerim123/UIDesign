-- Keep Staff Groups authoritative while preserving the legacy constrained user_profiles.role field.
create or replace function public.admin_save_staff_member(
  ...
  p_status text,
  p_group_ids uuid[],
  p_primary_group_id uuid default null
)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  values(auth.uid(),case when existing_staff then 'staff.member_updated' else 'staff.member_saved' end,'staff_member',p_user_id,
    jsonb_build_object('groups',p_group_ids,'primary_group_id',primary_id,'primary_group_slug',primary_slug,'legacy_role',legacy_role,'status',p_status));
