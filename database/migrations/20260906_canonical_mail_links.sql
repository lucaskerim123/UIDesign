-- Remove the previous Store hostname from database-driven mail dispatch/link generation.
...
declare
  ...
  v_site text;
begin
  select rtrim(coalesce(value #>> '{}','https://orbitfsstore.vercel.app'),'/')
    into v_site
    from public.app_settings
    where key='site.public_url';
  v_site:=coalesce(nullif(v_site,''),'https://orbitfsstore.vercel.app');
...
begin
  ...
  v_vars:=coalesce(e.payload,'{}'::jsonb)||jsonb_build_object('customer_name',v_name);
