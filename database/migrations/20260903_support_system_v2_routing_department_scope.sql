-- Restrict ticket routing choices to the departments the current support supervisor can actually manage.
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object('id',d.id,'name',d.name,'description',d.description,'enabled',d.enabled) order by d.sort_order,d.name),'[]'::jsonb)
  into deps
  from public.support_departments d
  where d.enabled=true
    and (actor_rank>=3 or public.support_user_has_department(auth.uid(),d.id));

  select coalesce(jsonb_agg(jsonb_build_object(
      'user_id',sm.user_id,
      'display_name',coalesce(up.display_name,trim(concat_ws(' ',up.first_name,up.last_name)),sm.user_id::text),
      'title',sm.title,
      'rank',public.support_staff_rank(sm.user_id),
      'rank_label',public.support_staff_rank_label(public.support_staff_rank(sm.user_id))
    ) order by public.support_staff_rank(sm.user_id) desc,coalesce(up.display_name,sm.user_id::text)),'[]'::jsonb)
    into people
  from public.staff_members sm
  left join public.user_profiles up on up.id=sm.user_id
  where sm.status='active'
    and public.support_staff_rank(sm.user_id)>t.escalation_level
    and (public.support_staff_rank(sm.user_id)>=3 or public.support_user_has_department(sm.user_id,t.department_id));

  return jsonb_build_object(
    'ticket',jsonb_build_object('id',t.id,'department_id',t.department_id,'assigned_to',t.assigned_to,'escalation_level',t.escalation_level,'escalation_reason',t.escalation_reason,'escalated_at',t.escalated_at,'escalated_by',t.escalated_by),
    'actor_rank',actor_rank,
    'actor_rank_label',public.support_staff_rank_label(actor_rank),
    'departments',deps,
    'staff',people
  );
end $$;
