...
async function configureVercel(install:any){const key=await publishableKey(install),secret=await installationSecret(install.id,"db_secret");if(!secret)throw new Error("OrbitFS database secret is missing");await upsertVercelEnv(install,"SUPABASE_URL",`https://${install.supabase_project_ref}.supabase.co`);await upsertVercelEnv(install,"SUPABASE_PUBLISHABLE_KEY",key);await upsertVercelEnv(install,"ORBITFS_DB_SECRET",secret)}
