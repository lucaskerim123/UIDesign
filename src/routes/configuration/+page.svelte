<script lang="ts">
	...
	const dedicatedSecret = data.backend.engineSecretSource === 'dedicated';
	const health = [
		...
		{ label: 'Panel pairing', ok: data.backend.engineSecretConfigured, detail: dedicatedSecret ? 'Dedicated Engine secret configured' : data.backend.engineSecretConfigured ? 'Using database-secret fallback' : 'Pairing secret is missing' },
		{ label: 'Installation', ok: Boolean(data.installationId), detail: data.installationId },
		{ label: 'Licence state', ok: data.license.licensed, detail: data.license.licensed ? 'Installation entitlement is active' : `Licence state: ${data.license.status || 'unknown'}` }
	];
...
<div class="app">
	...
	<main>
		<section class="heading">
			<div><p class="eyebrow">ENGINE HOST CONFIGURATION</p><h1>OrbitFS Engine Host</h1><p>The real cloud configuration used by MCP, APEX and Studio. Deployment-owned settings are shown as deployment facts; engine-owned settings remain editable inside each engine.</p></div>
		...
		<section class="status-grid">
			{#each health as item}<div class:warn={!item.ok} class="status-card"><div class="status-line"><span class:ok={item.ok}></span><b>{item.label}</b></div><p>{item.detail}</p></div>{/each}
		</section>

		<section class="panel">
			<div class="panel-head"><div><p class="eyebrow">CURRENT DEPLOYMENT</p><h2>What is running right now</h2></div><span class:preview={!data.deployment.canonical} class="pill">{data.deployment.environment}</span></div>
			<div class="deployment-grid">
				<div><span>Environment</span><b>{data.deployment.environment}</b><small>{data.deployment.environment === 'preview' ? 'Safe branch preview. This is not the production deployment.' : 'Current Vercel environment.'}</small></div>
				<div><span>Git branch</span><b>{data.deployment.branch || 'Not reported'}</b><small>Vercel branch currently serving this deployment.</small></div>
				<div><span>Current URL</span><code>{data.deployment.currentOrigin}</code><small>The deployment you are viewing now.</small></div>
				<div><span>Canonical Engine Host</span><code>{data.deployment.canonicalOrigin}</code><small>{data.deployment.canonical ? 'You are on the canonical host.' : 'This preview will become canonical only when you switch Vercel production to v2.'}</small></div>
			</div>
		</section>
		...
		<section class="panel">
			...
			<p class="note">These are service addresses, not user preferences. Engine Host shows them clearly so you always know which site owns which job.</p>
			<div class="service-grid">
				<div><small>MAIN ORBITFS / PANEL</small><b>Panel authority</b><code>{data.services.panelUrl}</code><p>Users, workspaces, Library, Profiles, Projects, OSS/CCS, permissions and main OrbitFS administration.</p></div>
				<div><small>ENGINE HOST</small><b>Engine management</b><code>{data.services.engineHostUrl}</code><p>MCP, APEX and Studio setup, runtime, configuration, monitoring, logs and diagnostics.</p></div>
				<div><small>MCP TRANSPORT</small><b>Client endpoint</b><code>{data.services.mcpUrl}</code><p>ChatGPT, Cursor and other MCP clients connect here.</p></div>
				<div><small>LICENSING</small><b>Current entitlement provider</b><code>{data.services.licenseProvider}</code><p>Read from the shared OrbitFS licence state rather than a separate Engine Host account system.</p></div>
			</div>
		</section>
		...
		<section class="split">
			<div class="panel">
				...
				<div class="facts">
					...
					<div><span>Database</span><b>{data.backend.database}</b><small>Shared accounts, workspaces, engine state and configuration.</small></div>
					<div><span>Storage</span><b>{data.backend.storage}</b><small>Object storage used by OrbitFS Library content where required.</small></div>
					<div><span>Filesystem model</span><b>{data.backend.filesystem}</b><small>No persistent VPS filesystem is recreated here.</small></div>
				</div>
			</div>
			<div class="panel">
				<div class="panel-head"><div><p class="eyebrow">PAIRING SECURITY</p><h2>Panel ↔ Engine Host</h2></div><span class:bad={!dedicatedSecret} class="pill">{dedicatedSecret ? 'dedicated secret' : data.backend.engineSecretConfigured ? 'fallback active' : 'action required'}</span></div>
				<p class="note">Attach/detach is server-to-server. Engine Host validates the signed request, installation, workspace, acting user and entitlement before accepting the link.</p>
				...
				{#if !data.backend.engineSecretConfigured}<div class="warning"><b>Pairing is not ready.</b><span>Set the same <code>ORBITFS_ENGINE_SECRET</code> in both Vercel projects before testing Attach.</span></div>{:else if !dedicatedSecret}<div class="warning"><b>Pairing works, but is using the database-secret fallback.</b><span>Before production, set a dedicated <code>ORBITFS_ENGINE_SECRET</code> in Panel and Engine Host so pairing does not reuse the database secret.</span></div>{/if}
			</div>
		</section>
		...
		<section class="panel">
			...
			<p class="note">Only settings with real backend behaviour are exposed. Panel-owned workspace systems stay in Panel and are referenced through the shared backend.</p>
...
<style>
	...
	.panel{border:1px solid #252e3a;background:linear-gradient(180deg,#0e141c,#0a0f15);border-radius:16px;padding:21px;margin-top:16px}.panel-head{display:flex;align-items:center;justify-content:space-between;gap:15px}.panel h2{margin:0;font-size:21px}.pill{font-size:9px;border:1px solid #35404e;border-radius:999px;padding:6px 9px;color:#a9b3bf;text-transform:capitalize}.pill.bad,.pill.preview{border-color:#6b4b18;color:#f0c36b}.note{font-size:10px;color:#758190;line-height:1.65;margin:12px 0 16px}.deployment-grid,.service-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:10px}.deployment-grid>div,.service-grid>div{border:1px solid #242d39;background:#0a0f15;border-radius:12px;padding:14px;display:grid;gap:5px}.deployment-grid span,.service-grid small{font-size:9px;color:#6f7a89}.deployment-grid b,.service-grid b{font-size:12px}.deployment-grid code,.service-grid code{font-size:10px;color:#bcb1e8;word-break:break-all}.deployment-grid small,.service-grid p{font-size:9px;color:#758190;line-height:1.55;margin:4px 0 0}
