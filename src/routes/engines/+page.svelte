...
<div class="app">
	<header>
		...
		<div class="account">{#if data.canManage}<a class="config-link" href="/configuration">Configuration</a>{/if}<div><b>{data.user.display_name || data.user.username}</b><small>{data.mainWorkspace?.name || 'No workspace'}</small></div><form method="POST" action="?/logout"><button>Sign out</button></form></div>
	</header>
	<main>
		<section class="hero">
			<div><p class="eyebrow">ENGINE SELECTOR</p><h1>Which engine would you like to manage?</h1><p>Only engines you can access through your OrbitFS workspace permissions are shown. Panel owns workspace permissions and data; Engine Host provides the runtime and engine-specific management layer.</p></div>
			<div class="installation"><span>Installation</span><code>{data.installationId}</code><small>{data.workspaceCount} accessible workspace{data.workspaceCount === 1 ? '' : 's'}</small>{#if data.canManage}<a href="/configuration">View host configuration →</a>{/if}</div>
		</section>
			...
			<section class="grid">
				{#each data.engines as engine}
					<a class:disabled={!engine.registered || !engine.licensed || !engine.attached} class="engine" href={engine.registered ? `/engines/${engine.id}` : '/engines'}>
						...
						<div class="access"><span>Access</span><b>{engine.accessWorkspaceCount} workspace{engine.accessWorkspaceCount === 1 ? '' : 's'}</b><small>{engine.accessWorkspaceNames?.join(', ') || 'No permitted workspace'}</small></div>
						<div class="foot"><span>{engine.workspaceName || data.mainWorkspace?.name || 'Workspace not linked'}</span><b>Manage →</b></div>
					</a>
				{/each}
			</section>
		{:else}
			<section class="empty"><b>No Engine Host access is currently assigned.</b><span>Ask a workspace owner or administrator to grant the relevant MCP, APEX or Studio workspace permission in OrbitFS Panel.</span></section>
		{/if}
	</main>
</div>
...
<style>
	...
	.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:18px}.engine{min-height:326px;display:flex;flex-direction:column;box-sizing:border-box;padding:22px;border:1px solid #293241;border-radius:18px;background:linear-gradient(180deg,#111721,#0b1017);text-decoration:none;color:inherit;transition:.18s ease}.engine:hover{transform:translateY(-2px);border-color:#4a5363;box-shadow:0 18px 45px rgba(0,0,0,.28)}.engine.disabled{opacity:.62}.top{display:flex;align-items:center;justify-content:space-between}.icon{width:42px;height:42px;display:grid;place-items:center;border-radius:11px;background:#1d1834;color:#bfa8ff;font-weight:900}.badge{border:1px solid #354050;border-radius:999px;padding:6px 9px;font-size:10px;color:#9ea8b5}.badge.ok{border-color:#215a40;color:#6ee7a6;background:#0b2118}.badge.warn{border-color:#6b4b18;color:#f0c36b;background:#241a09}.engine h2{font-size:24px;margin:22px 0 7px}.engine>p{color:#8e99a8;line-height:1.55;margin:0 0 20px;min-height:48px}.facts{display:grid;grid-template-columns:repeat(3,1fr);border:1px solid #222b37;border-radius:12px;overflow:hidden}.facts span{padding:10px;border-right:1px solid #222b37;display:grid;gap:3px}.facts span:last-child{border-right:0}.facts small{color:#677383;font-size:9px;text-transform:uppercase}.facts b{font-size:11px;text-transform:capitalize}.access{margin-top:10px;border:1px solid #222b37;border-radius:10px;padding:9px 10px;display:grid;grid-template-columns:auto 1fr;gap:2px 10px}.access span{font-size:9px;color:#677383;text-transform:uppercase}.access b{font-size:10px;text-align:right}.access small{grid-column:1/3;font-size:9px;color:#758190;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.foot{margin-top:auto;padding-top:17px;display:flex;justify-content:space-between;gap:12px;font-size:12px;color:#737f8e}.foot b{color:#bca9ff}.empty{border:1px dashed #33404e;background:#0b1017;border-radius:16px;padding:26px;display:grid;gap:7px;text-align:center}.empty b{font-size:14px}.empty span{font-size:11px;color:#7f8b99}
