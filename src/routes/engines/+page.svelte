...
<div class="app">
	<header>
		...
		<div class="account"><div><b>{data.user.display_name || data.user.username}</b><small>{data.mainWorkspace?.name || 'No workspace'}</small></div><form method="POST" action="?/logout"><button>Sign out</button></form></div>
	</header>
	<main>
		<section class="hero">
			<div><p class="eyebrow">ENGINE SELECTOR</p><h1>Which engine would you like to manage?</h1><p>Engine Host is linked to your existing OrbitFS installation. Panel data stays in Panel; this site manages engine-specific setup, runtime and diagnostics.</p></div>
			<div class="installation"><span>Installation</span><code>{data.installationId}</code><small>{data.workspaceCount} accessible workspace{data.workspaceCount === 1 ? '' : 's'}</small></div>
		</section>
		<section class="grid">
...
<style>
	...
	@media(max-width:900px){.grid{grid-template-columns:1fr}.hero{align-items:stretch;flex-direction:column}.installation{min-width:0}.hero h1{font-size:34px}}@media(max-width:560px){header{padding:0 16px}.account div{display:none}main{padding:36px 16px 55px}.hero h1{font-size:30px}.engine{padding:18px}.facts{grid-template-columns:1fr}.facts span{border-right:0;border-bottom:1px solid #222b37}.facts span:last-child{border-bottom:0}}
</style>
