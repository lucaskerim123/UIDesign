<script lang="ts">
	...
	const nav = [
		...
		...(data.canManage ? adminNav.slice(0, engine.id === 'mcp' ? 4 : 3) : []),
		['Runtime', `/engines/${engine.id}/runtime`],
		...(data.canManage ? adminNav.slice(engine.id === 'mcp' ? 4 : 3) : [])
	];
	const adminFlow = [
		...
		['3','Connections',engine.id === 'mcp' ? 'MCP clients, sessions and Panel binding.' : 'Panel binding and real runtime connections.',`/engines/${engine.id}/connections`],
...
<div class="app">
	<header>
		...
		<div class="head-actions">{#if data.canManage}<a href="/configuration">Host configuration</a>{/if}{#if engine.panelUrl}<a href={engine.panelUrl}>Open Panel</a>{/if}<a href="/engines">← All engines</a></div>
	</header>
	<div class="layout">
		<aside>
			...
			<div class="side-status"><span>Workspace link</span><b>{engine.workspaceName || 'Not linked'}</b><span>Your engine access</span><b>{data.accessWorkspaces.length} workspace{data.accessWorkspaces.length === 1 ? '' : 's'}</b><span>Panel</span><b>{engine.panelUrl || 'Not linked'}</b><span>Storage</span><b>Supabase / shared</b></div>
		</aside>
		<main>
			...
			{#if !engine.licensed}<div class="alert warn"><b>Licence required.</b> Entitlement <code>{engine.component}</code> is {engine.licenseState || 'not included'}{engine.licenseReason ? ` (${engine.licenseReason})` : ''}.</div>{/if}
			...
			<section class="cards">
				...
				<div class="card"><small>ENTITLEMENT</small><strong>{engine.licenseState || 'unknown'}</strong><span>{engine.component}</span></div>
			</section>

			{#if data.canManage}
				<section class="panel">
					<div class="panel-head"><div><p class="eyebrow">MANAGEMENT FLOW</p><h2>Engine Host responsibilities</h2></div></div>
					<div class="flow">{#each adminFlow as item}<a href={item[3]}><b>{item[0]}</b><span><strong>{item[1]}</strong><small>{item[2]}</small></span></a>{/each}</div>
				</section>
			{:else}
				<section class="panel">
					<div class="panel-head"><div><p class="eyebrow">YOUR ACCESS</p><h2>Workspace permission</h2></div><span class="pill">{data.accessWorkspaces.length} workspace{data.accessWorkspaces.length === 1 ? '' : 's'}</span></div>
					<p class="access-note">Your Engine Host access comes directly from OrbitFS Panel workspace permissions. Engine configuration, pairing, diagnostics and administrative telemetry stay restricted to system administrators.</p>
					<div class="workspace-list">{#each data.accessWorkspaces as workspace}<div><b>{workspace.name}</b><span>{workspace.permission}</span></div>{/each}</div>
				</section>
			{/if}
			...
			<section class="panel">
				...
				<div class="details"><span>Panel URL</span><code>{engine.panelUrl || 'Not linked'}</code><span>Paired workspace</span><code>{engine.workspaceName || engine.workspaceId || 'Not linked'}</code><span>Your accessible workspaces</span><code>{data.accessWorkspaces.map((workspace:any)=>workspace.name).join(', ') || 'None'}</code><span>Last sync</span><code>{engine.lastSyncAt || 'Never'}</code>{#if engine.transportPath}<span>Transport</span><code>{engine.transportPath}</code>{/if}</div>
			</section>
		</main>
	</div>
</div>
