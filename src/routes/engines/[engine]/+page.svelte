...
<div class="app">
	<header>
		...
		<div class="head-actions">{#if engine.panelUrl}<a href={engine.panelUrl}>Open Panel</a>{/if}<a href="/engines">← All engines</a></div>
	</header>
	<div class="layout">
		<aside>
		...
		<main>
			...
			<section class="panel">
				...
				<div class="flow">
					<a href={`/engines/${engine.id}/setup`}><b>1</b><span><strong>First-time setup</strong><small>Validate Panel pairing, licence, workspace and runtime readiness.</small></span></a>
					<a href={`/engines/${engine.id}/configuration`}><b>2</b><span><strong>Configuration</strong><small>Engine-specific configuration only. Panel-owned systems stay in Panel.</small></span></a>
					<a href={`/engines/${engine.id}/connections`}><b>3</b><span><strong>Connections</strong><small>{engine.id === 'mcp' ? 'MCP clients, sessions and Panel binding.' : 'Panel binding and engine-specific providers.'}</small></span></a>
					<a href={`/engines/${engine.id}/runtime`}><b>4</b><span><strong>Runtime</strong><small>Run, Standby, Restart or Stop independently from setup.</small></span></a>
					<a href={`/engines/${engine.id}/diagnostics`}><b>5</b><span><strong>Diagnostics</strong><small>Read-only readiness, deployment and pairing health.</small></span></a>
				</div>
			</section>
