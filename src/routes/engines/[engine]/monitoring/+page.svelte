...
<div class="app">
	<header><a class="brand" href="/engines"><span class="mark">O</span><span><b>OrbitFS</b><small>Engine Host</small></span></a><div class="crumb"><a href={`/engines/${engine.id}`}>{engine.name}</a><span>/</span><b>Monitoring</b></div></header>
	<main>
		<div class="heading"><p class="eyebrow">MONITORING</p><h1>{engine.fullName}</h1><p>Live state from the shared OrbitFS backend. Engine Host reads runtime and connection activity without duplicating Panel data.</p></div>
		<section class="cards">{#each cards as card}<div><small>{card[0]}</small><strong>{card[1]}</strong><span>{card[2]}</span></div>{/each}</section>
		<section class="panel">
			<div class="panel-head"><div><p class="eyebrow">READINESS</p><h2>Engine health checks</h2></div><span class:ok={data.readiness.ready} class="pill">{data.readiness.ready ? 'Ready' : 'Needs attention'}</span></div>
			<div class="checks">{#each data.readiness.checks as check}<div><span class:ok={check.ok} class="dot"></span><div><b>{check.label}</b><small>{check.description}</small></div><strong>{check.ok ? 'Pass' : 'Block'}</strong></div>{/each}</div>
		</section>
		<section class="panel">
