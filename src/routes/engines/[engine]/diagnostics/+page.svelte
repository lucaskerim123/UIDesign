...
<div class="app">
	<header>
		...
		<div class="crumb"><a href="/configuration">Host configuration</a><span>/</span><a href={`/engines/${engine.id}`}>{engine.name}</a><span>/</span><b>Diagnostics</b></div>
	</header>
	<main>
		<section class="heading"><div><p class="eyebrow">DIAGNOSTICS</p><h1>{engine.fullName}</h1><p>Read-only health and pairing details. Required failures block engine readiness; recommendations are production hardening items and do not stop setup.</p></div><div class:ready={data.ready} class="health"><span></span><b>{data.ready ? 'Required checks pass' : 'Attention required'}</b></div></section>
		...
		<section class="grid">
			<div class="panel">
				...
				<div class="checks">{#each data.checks as check}<div class:ok={check.ok} class:optional={!check.required && !check.ok}><b>{check.ok ? '✓' : check.required ? '!' : 'i'}</b><span><strong>{check.label}</strong><small>{check.description}</small></span><em class:recommended={!check.required && !check.ok}>{resultLabel(check)}</em></div>{/each}</div>
			</div>
			<aside>
				...
				<div class="card"><span>Last Panel sync</span><b>{engine.lastSyncAt ? new Date(engine.lastSyncAt).toLocaleString() : 'Never'}</b><code>{engine.workspaceName || 'No workspace assigned'}</code></div>
				{#if engine.transportPath}<div class="card"><span>Transport</span><b>{engine.transportPath}</b><code>Protocol endpoint</code></div>{/if}
			</aside>
		</section>
		<div class="actions"><a href={`/engines/${engine.id}/setup`}>Setup</a><a href={`/engines/${engine.id}/configuration`}>Configuration</a><a href={`/engines/${engine.id}/connections`}>Connections</a><a href={`/engines/${engine.id}/runtime`}>Runtime</a></div>
	</main>
</div>
...
<style>
	...
	.grid{display:grid;grid-template-columns:minmax(0,1fr) 280px;gap:16px;margin-top:24px}.panel,.card{border:1px solid #252e3a;background:linear-gradient(180deg,#0e141c,#0a0f15);border-radius:16px}.panel{padding:21px}.panel-head{display:flex;align-items:center;justify-content:space-between}.panel h2{margin:0;font-size:21px}.pill{font-size:10px;border:1px solid #374151;border-radius:999px;padding:6px 9px;color:#a9b3bf}.checks{display:grid;gap:8px;margin-top:16px}.checks>div{display:grid;grid-template-columns:30px 1fr auto;gap:10px;align-items:center;border:1px solid #26303c;background:#090e14;border-radius:11px;padding:11px}.checks>div>b{width:28px;height:28px;border-radius:8px;display:grid;place-items:center;background:#281b09;color:#efc06a}.checks>div.ok>b{background:#0e3020;color:#67e8a2}.checks>div.optional>b{background:#22183d;color:#bca7ff}.checks>div.optional{border-color:#40345d}.checks span{display:grid}.checks strong{font-size:12px}.checks small{font-size:10px;color:#748090;margin-top:2px;line-height:1.45}.checks em{font-style:normal;font-size:9px;text-transform:uppercase;color:#d6a84f}.checks .ok em{color:#63d99a}.checks em.recommended{color:#a78bfa}aside{display:grid;gap:10px;align-content:start}.card{padding:14px;display:grid;gap:4px;overflow:hidden}.card span{font-size:9px;color:#687484;text-transform:uppercase}.card b{font-size:11px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.card code{font-size:9px;color:#7f8998;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.actions{display:flex;gap:8px;flex-wrap:wrap;margin-top:16px}.actions a{border:1px solid #303a47;background:#0a0f15;color:#b8c1cc;text-decoration:none;border-radius:8px;padding:8px 10px;font-size:9px}
