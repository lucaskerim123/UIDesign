<script lang="ts">
	...
	const reviewed = Boolean(engine.configurationReviewedAt);
	const configText = engine.id === 'mcp'
		? 'Review MCP OAuth, connections, startup/context ownership and runtime defaults.'
		: engine.id === 'apex'
			? 'Review APEX service mode, automation, standby and shutdown policy.'
			: 'Review Studio routing, automatic analysis and confidence thresholds.';
</script>
...
<div class="app">
	...
	<main>
		<section class="heading">
			<div><p class="eyebrow">FIRST-TIME SETUP</p><h1>{engine.fullName}</h1><p>Confirm the Panel pairing, review real engine configuration and pass readiness checks before this engine becomes setup-complete.</p></div>
		...
		<section class="flow">
			<div class:active={!engine.linked} class:done={engine.linked}><b>1</b><span><strong>Attach from Panel</strong><small>Panel confirms the licence and pairs Engine Host to the installation and workspace.</small></span></div>
			<div class:active={engine.linked && !reviewed} class:done={reviewed}><b>2</b><span><strong>Review configuration</strong><small>{configText}</small></span></div>
			<div class:active={reviewed && !data.ready} class:done={data.ready || engine.setupState === 'complete'}><b>3</b><span><strong>Validate readiness</strong><small>Check shared backend, runtime, workspace, licence and engine-specific requirements.</small></span></div>
			<div class:active={data.ready && engine.setupState !== 'complete'} class:done={engine.setupState === 'complete'}><b>4</b><span><strong>Complete setup</strong><small>Finish onboarding. Runtime can still remain in Standby independently.</small></span></div>
		</section>

		<div class="layout">
			<div class="main-column">
				<section class="panel">
					<div class="panel-head"><div><p class="eyebrow">CONFIGURATION REVIEW</p><h2>{reviewed ? 'Configuration reviewed' : 'Review engine configuration'}</h2></div><span class:ready={reviewed} class="pill">{reviewed ? 'Recorded' : 'Required'}</span></div>
					<p class="muted">{configText}</p>
					<div class="review-links">
						<a class="primary-link" href={`/engines/${engine.id}/configuration`}>Open Configuration</a>
						{#if engine.id === 'mcp'}<a href={`/engines/${engine.id}/oauth`}>Review OAuth</a><a href={`/engines/${engine.id}/connections`}>Review Connections</a>{/if}
						<a href={`/engines/${engine.id}/runtime`}>Review Runtime</a>
					</div>
					{#if data.canManage && engine.linked && !reviewed}<form method="POST" action="?/review" class="review-form"><button class="primary">Confirm configuration reviewed</button></form>{/if}
					{#if reviewed}<div class="reviewed-at">Recorded {new Date(engine.configurationReviewedAt).toLocaleString()}</div>{/if}
				</section>
				...
				<section class="panel">
					...
					{#if data.canManage}
						<div class="actions">
							...
							{:else}
								{#if engine.setupState !== 'in_progress'}<form method="POST" action="?/begin"><button>Begin setup</button></form>{/if}
			...
			<aside>
				...
				<div class="card"><span>Configuration</span><b>{reviewed ? 'Reviewed' : 'Review required'}</b><small>{reviewed ? engine.configurationReviewedAt : 'Open engine configuration'}</small></div>
				<div class="card"><span>Runtime</span><b>{engine.engineState}</b><small>Setup and runtime are independent.</small></div>
				{#if engine.id === 'mcp'}<div class="card"><span>MCP transport</span><b>/mcp</b><small>https://orbitfsengine.vercel.app/mcp</small></div>{/if}
			</aside>
		</div>
	</main>
</div>
