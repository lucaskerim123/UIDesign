...
export default function Customer({params}:{params:Promise<{id:string}>}){
 ...
 const [orders,setOrders]=useState<any[]>([]),[invoices,setInvoices]=useState<any[]>([]),[tickets,setTickets]=useState<any[]>([]),[ledger,setLedger]=useState<any[]>([]),[bindings,setBindings]=useState<any[]>([]),[activity,setActivity]=useState<any[]>([]),[ipSummary,setIpSummary]=useState<any[]>([]),[audit,setAudit]=useState<any[]>([]);
 ...
 async function load(){
  const [a,customer,c,o,i,t,l,x,ac,au]=await Promise.all([
   ...
   sb.from("license_bindings").select("*").eq("auth_user_id",id).order("created_at",{ascending:false}),
 ...
 const banned=!!p.banned_at,tabs=[["general","General info / settings"],["orders","Orders"],["invoices","Invoices"],["support","Support tickets"],["licenses","Licences"],["notes","Notes"],["emails","Emails"],["history","History / IP details"]];
 ...
 return <main className="adminShell">
  ...
  {tab==="licenses"&&<section className="panel"><h2>Licences</h2>{bindings.length?bindings.map(x=><Link className="adminItem" href={`/admin/licenses/${x.id}`} key={x.id}><div><b>{x.label||"OrbitFS licence"}</b><span>{x.api_source||"legacy"} · {x.license_id||"not issued"} · {x.desired_state}</span></div><span>Manage →</span></Link>):<p className="muted">No licences.</p>}</section>}
