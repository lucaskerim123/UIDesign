...
export default function Portal(){
 ...
 return <div className="portalOverviewV2">
  <header className="portalOverviewHero"><div><p className="eyebrow">{String(portal).toUpperCase()}</p><h1>Welcome back, {d.profile?.first_name||d.profile?.display_name||"Customer"}</h1><p className="muted">Your OrbitFS account at a glance.</p></div><Link className="buttonlink secondary" href="/portal/settings">Account settings</Link></header>
  ...
  <div className="portalOverviewGrid">
   <section className="panel portalActivityPanel portalOverviewActivity"><div className="portalActivityHeader"><div><p className="eyebrow">ACCOUNT ACTIVITY</p><h2>{meta[tab].title}</h2></div><Link href={meta[tab].href}>{meta[tab].action}</Link></div><div className="portalActivityTabs" role="tablist"><button className={tab==="orders"?"active":""} onClick={()=>setTab("orders")}>Orders</button><button className={tab==="invoices"?"active":""} onClick={()=>setTab("invoices")}>Invoices</button><button className={tab==="support"?"active":""} onClick={()=>setTab("support")}>Support</button></div><div className="portalActivityBody">
    {tab==="orders"&&<>{recentActiveOrders.length?recentActiveOrders.map((o:any)=><Link className="listrow" href={`/portal/orders/${o.id}`} key={o.id}><b>#{o.order_number}</b><span>{o.status} · {money(o.total_cents)}</span></Link>):<p className="muted">No active orders.</p>}</>}
    {tab==="invoices"&&<>{d.invoices.length?d.invoices.map((i:any)=><Link className="listrow" href={`/portal/invoices/${i.id}`} key={i.id}><b>{i.invoice_number}</b><span>{i.status} · {money(i.total_cents)}</span></Link>):<p className="muted">No invoices yet.</p>}</>}
    {tab==="support"&&<>{d.tickets.length?d.tickets.map((t:any)=><Link className="listrow" href={`/portal/support/${t.id}`} key={t.id}><b>#{t.ticket_number} {t.subject}</b><span>{String(t.status||"").replaceAll("_"," ")}</span></Link>):<p className="muted">No support tickets.</p>}</>}
   </div></section>
   <section className="panel portalQuickActions"><div><p className="eyebrow">QUICK ACCESS</p><h2>Common actions</h2></div><Link href="/portal/products"><b>Browse OrbitFS Store</b><span>Products and add-ons</span></Link><Link href="/portal/settings#wallet"><b>Manage Wallet</b><span>Add funds, redeem coupons, view activity</span></Link><Link href="/portal/support"><b>Get support</b><span>Tickets and support messages</span></Link></section>
  </div>
