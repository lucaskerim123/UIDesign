...
export default function SupportPage(){
  ...
  async function load(){
    ...
    setUser(u||null);
    const d=await sb.from("support_departments").select("id,name,description").eq("enabled",true).order("sort_order");
  ...
  async function submit(e:FormEvent<HTMLFormElement>){
    e.preventDefault();setSubmitting(true);setMessage("Opening ticket…");setCreated(null);
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.hero}>
      <div><p className={styles.eyebrow}>OrbitFS Support</p><h1>{guest?<>Support without <span>needing an account.</span></>:<>Your OrbitFS <span>support centre.</span></>}</h1><p className={styles.lead}>{guest?"Open a basic support request with General Support or Sales. You’ll receive a private ticket code so you can come back, read replies and continue the conversation without signing in.":"Open a new request, track your active tickets and continue conversations with OrbitFS Support."}</p></div>
