...
OUT="OrbitFS-Billing-License-Export-$(date +%Y%m%d-%H%M%S)"
mkdir -p "$OUT"
...
configuration/templates, payment gateway definitions/setup metadata, license
API/release-system settings, themes and related defaults.

Customer accounts, orders, invoices, payments, tickets, audit logs, sessions,
license bindings/installations/runtime logs, webhook events and other runtime
